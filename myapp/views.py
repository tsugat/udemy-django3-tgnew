...
class PostCreate(LoginRequiredMixin, CreateView):
    # LoginRequiredMixinはログインしていないと、実施できないことを宣言している。
    model = Post #Postモデルを使うことを定義
    form_class = PostForm
    success_url = reverse_lazy('myapp:index')

    # ログイン中のユーザをセットするための記述
    def form_valid(self, form):
        form.instance.author_id = self.request.user.id
        #formにはタイトルと内容だけがセットされていたが、formにユーザをセットしている。
